...
const createDonationRecord = async ({ user, charity, payload }) => {
  const donation = new Donation({
    ...
    status: 'succeeded', // automatically approved
...
const createOneTimeDonation = async (req, res) => {
  try {
    if (req.user.isAdmin) {
      return res.status(403).json({ message: "Admins are not allowed to make donations" });
    }
    ...
    if (goal) {
      ...
      if (remainingAmount <= 0) {
        return res.status(400).json({ message: "This project is already fully funded." });
      }
      if (Number(amount) > remainingAmount) {
        return res.status(400).json({ message: `Only $${remainingAmount} remaining.` });
      }
    }
...
const createRecurringDonation = async (req, res) => {
  try {
    ...
    if (!stripeReady) {
      ...
      await donation.save();
      return res.status(201).json({ donation, stripe: { configured: false } });
    }
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              metadata: { charityId: charity._id.toString() },
            },
    ...
    return res.status(error.status || 500).json({ message: error.message || 'Unable to create recurring donation' });
  }
};
    ...
    const summary = await Donation.aggregate([
      {
        $group: {
          ...
          donationCount: { $sum: 1 },
          recurringCount: { $sum: { $cond: [{ $ne: ['$frequency', 'one-time'] }, 1, 0] } },
        },
      },
      {
        $lookup: {
